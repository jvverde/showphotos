...
(function () {
    ...
    function processSpecies() {
        ...
        function findSpeciesInText(root) {
            ...
            if (observer) {
                console.log('Disconnecting observer to prevent loops.');
                observer.disconnect();
            }
    ...
    function fetchFlickrImages(species, callback) {
        if (!FLICKR_API_KEY) {
            console.error('Flickr API Key is missing.');
            alert('Please enter a valid Flickr API Key!');
            return;
        }
        ...
        GM_xmlhttpRequest({
            ...
            onload: function (response) {
                if (response.status >= 200 && response.status < 300) {
                    try {
                        console.log('Flickr API response received.');
                        const data = JSON.parse(response.responseText);
                        if (data.photos && data.photos.photo.length > 0) {
                            console.log(`Found ${data.photos.photo.length} images for species: ${species}`);
