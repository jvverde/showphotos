...
// @version      3.0
// @description  Show Flickr images when hovering over bird species names with perfect popup positioning.
// @author       Your Name
// @match        *://*/*
...
(function () {
    ...
    let lastApiCallTime = 0; // For rate limiting
    const API_CALL_DELAY = 1000; // 1 second delay between API calls
    // Store the cursor position when the popup is shown
    ...
    async function fetchFlickrImages(species, callback) {
        ...
        try {
            ...
            // Prepare image data without author information
            const images = photos.map(photo => ({
                url: `https://farm${photo.farm}.staticflickr.com/${photo.server}/${photo.id}_${photo.secret}_c.jpg`,
                title: photo.title,
                author: null, // Placeholder for author
                flickrPage: `https://www.flickr.com/photos/${photo.owner}/${photo.id}`
            }));

            // Call the callback immediately with the image data
            callback(images);

            // Fetch owner names asynchronously and update the popup
            photos.forEach(async (photo, index) => {
                const ownerName = await fetchOwnerName(photo.owner);
                images[index].author = ownerName; // Update the author field

                // If this is the currently displayed image, update the popup
    ...
    `;
        ...
        // Wait for the image to load before positioning the popup
        ...
        // Recalculate popup position on scroll or resize
    ...
    function positionPopup(popup) {
        ...
        const cursorY = cursorPosition.y + scrollY; // Adjust for scroll position
        ...
        let top;
        if (spaceBelow >= 0) {
            // Enough space below: place popup below the cursor
            top = cursorY + 10;
        } else if (spaceAbove >= 0) {
            // Not enough space below but enough above: place popup above the cursor
            top = cursorY - popupHeight - 10;
        } else {
            // Not enough space above or below: place popup at the bottom of the viewport
